#!/usr/bin/env node
// =====================================================
// 从 AI导航 (ai-nav.net) 抓取工具到草稿区
// 用法:
//   node scripts/import-ai-nav.mjs             # 默认抓取 50 条
//   node scripts/import-ai-nav.mjs --limit=30  # 指定抓取数量上限
// 说明:
...
//   - 结果写入 data/tools-draft.ts 的 source='ai-nav' 条目,后台审核入库
// =====================================================
...
const HOME = 'https://www.ai-nav.net/'
const SITEMAP_URL = 'https://www.ai-nav.net/sitemap.xml'
const SOURCE = 'ai-nav'
...
async function fetchText(url) {
  const res = await fetch(url, { headers: { 'user-agent': UA, 'accept-language': 'zh-CN,en;q=0.9' } })
...
// ai-nav 工具详情页路径形如 /tool/ /ai/ /detail/ /item/ /site/<slug>
const TOOL_RE = /\/(tool|ai|detail|item|site)\/[^/?#]/i
