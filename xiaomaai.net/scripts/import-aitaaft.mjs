#!/usr/bin/env node
// =====================================================
// 从 There's An AI For That (theresanaiforthat.com) 抓取工具到草稿区
// 用法:
//   node scripts/import-aitaaft.mjs             # 默认抓取 50 条
//   node scripts/import-aitaaft.mjs --limit=30  # 指定抓取数量上限
// 说明:
...
//   - 结果写入 data/tools-draft.ts 的 source='aitaaft' 条目,后台审核入库
// =====================================================
...
const HOME = 'https://theresanaiforthat.com/'
const SITEMAP_URL = 'https://theresanaiforthat.com/sitemap.xml'
const SOURCE = 'aitaaft'
...
const NOISE_RE = /\/(category|categories|tag|tags|page|blog|news|topic|topics|author|search|feed|rss|collection|collections|c|task|use-case|industry|newsletter)\//i
// aitaaft 工具详情页路径形如 /ai/<slug>
const TOOL_RE = /\/ai\/[^/?#]/i
